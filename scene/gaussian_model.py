...
class GaussianModel:
    ...
    def restore(self, model_args, training_args):
        ...
        try:
            ...
            print(f"Warning: Failed to load optimizer state (parameter groups mismatch), using fresh optimizer state. Error: {e}")
    ...
    def training_setup(self, training_args):
        ...
        kernel_params = list(self._deformation.get_velocity_kernel_parameters())
        if len(kernel_params) > 0:
            velocity_kernel_lr_init = getattr(training_args, 'velocity_kernel_lr_init', training_args.grid_lr_init)
            l.append({
                'params': kernel_params,
                'lr': velocity_kernel_lr_init * self.spatial_lr_scale,
                "name": "velocity_kernel"
            })
        ...
        if len(kernel_params) > 0:
            velocity_kernel_lr_init = getattr(training_args, 'velocity_kernel_lr_init', training_args.grid_lr_init)
            velocity_kernel_lr_final = getattr(training_args, 'velocity_kernel_lr_final', training_args.grid_lr_final)
            self.velocity_kernel_scheduler_args = get_expon_lr_func(
                lr_init=velocity_kernel_lr_init*self.spatial_lr_scale,
                lr_final=velocity_kernel_lr_final*self.spatial_lr_scale,
                lr_delay_mult=training_args.deformation_lr_delay_mult,
                max_steps=training_args.position_lr_max_steps
            )
        else:
            self.velocity_kernel_scheduler_args = None    
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if  "grid" in param_group["name"]:
                ...
                param_group['lr'] = lr
                # return lr
            elif param_group["name"] == "velocity_kernel":
                if hasattr(self, 'velocity_kernel_scheduler_args') and self.velocity_kernel_scheduler_args is not None:
                    lr = self.velocity_kernel_scheduler_args(iteration)
                    param_group['lr'] = lr
    ...
    def load_model(self, path):
        ...
        try:
            ...
            print("Model loaded successfully (some keys may be missing or mismatched)")
        except RuntimeError as e:
            print(f"Warning: Error loading model: {e}")
            model_dict = self._deformation.state_dict()
            filtered_dict = {}
            skipped_keys = []
            ...
            if filtered_dict:
                ...
                self._deformation.load_state_dict(model_dict, strict=False)
                print(f"Loaded {len(filtered_dict)}/{len(weight_dict)} matching parameters")
            if skipped_keys:
                print(f"Skipped {len(skipped_keys)} keys (see details above)")
    ...
    def load_ply(self, path, load_only_xyz=False):
        """
         PLY file
        
        Args:
            path: PLY filepath
            load_only_xyz:  True， xyz coordinate，attributesuse
        """
        ...
        if load_only_xyz:
            print(f"Loading only xyz coordinates from {path}, initializing other attributes with defaults")
            
            fused_color = RGB2SH(torch.ones((xyz.shape[0], 3), dtype=torch.float, device="cuda"))
            ...
            self._deformation_table = torch.gt(torch.ones((xyz.shape[0]), device="cuda"), 0)
            self._deformation = self._deformation.to("cuda")
        else:
